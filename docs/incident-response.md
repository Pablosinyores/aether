...
2. Check local Reth/Geth node if applicable:
   ```bash
...
3. Try alternative endpoints:
   ...
   ```
4. If all providers are down, wait — this is likely a network-wide issue
...
3. System will auto-resume when gas drops and state is manually reset:
   ```bash
...
2. Check recent trades:
   ...
   psql -h localhost -U aether -d aether \
       -c "SELECT * FROM trades WHERE created_at > NOW() - INTERVAL '24 hours' ORDER BY created_at DESC;"
   ```
3. Analyze losing trades:
   ...
   - Did a pool state change between detection and execution?
4. Check for systematic issues:
   ```bash
   journalctl -u aether-rust -u aether-go --since "24 hours ago" -p err
   ```
5. After root cause is identified and fixed:
   ```bash
   grpcurl -plaintext localhost:50051 aether.ControlService/SetState \
       -d '{"state": "RUNNING"}'
   ```
...
3. If graph is too large, prune cold pools:
   ```bash
   # Reduce pool count in pools.toml
